...
namespace Distance {
   class MyDistance {
	  ...
	  friend std::ostream& operator << (std::ostream& out, const MyDistance& a) {
               out << a.m << " m";
	       return out;
	  }
      private:
...
int main() {
    ...
    std::cout << "100 * 1.0_cm:   " << 100 * 1.0_cm << std::endl;
    ...
    std::cout << "4 * workPerDay - 3 abbrevationToWork + workout + shopping: " << myDistancePerWeek << std::endl;
    std::cout << std::endl;
